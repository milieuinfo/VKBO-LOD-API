...
public class PredicateGroupingTest {
  ...
  public void testPredicateGrouping() {
    ...
    String[] testLabels = {"type", "type", "label", "type"};

    String[] testCategories = {
      "Overige Eigenschappen", "Overige Eigenschappen",
      "Overige Eigenschappen", "Overige Eigenschappen"
    };
    ...
    boolean[] testIsLiteral = {false, false, true, false};
    ...
    for (int i = 0; i < testUris.length; i++) {
      ...
      Map<String, RDFPredicate> predicateMap =
          categoryPredicateMap.computeIfAbsent(category, k -> new LinkedHashMap<>());
    ...
    RDFPredicate typePredicate =
        predicates.get("http://www.w3.org/1999/02/22-rdf-syntax-ns#type");
    RDFPredicate labelPredicate =
        predicates.get("http://www.w3.org/2000/01/rdf-schema#label");
