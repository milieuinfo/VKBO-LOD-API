...
public class RecursiveBlankNodeTest {
    ...
    /**
     * Simulate recursive extraction of blank node properties
     */
    private List<RDFPredicate> extractBlankNodeProperties(String blankNodeId, int depth) {
        ...
        // Simulate some properties for this blank node
        if (blankNodeId.equals("registration")) {
            // Registration blank node properties
            RDFObject notationObj = new RDFObject();
            notationObj.isLiteral = true;
            notationObj.value = "2105029959";
            notationObj.label = "2105029959";
            properties.add(new RDFPredicate("http://www.w3.org/2004/02/skos/core#notation", "notation", notationObj));
            ...
            properties.add(new RDFPredicate("http://www.w3.org/1999/02/22-rdf-syntax-ns#type", "type", typeObj));
            
        } else if (blankNodeId.equals("address")) {
            // Address blank node properties
            ...
            properties.add(new RDFPredicate("http://www.w3.org/ns/adres#straatnaam", "street name", streetObj));
            
            RDFObject numberObj = new RDFObject();
            numberObj.isLiteral = true;
            numberObj.value = "33";
            numberObj.label = "33";
            properties.add(new RDFPredicate("http://www.w3.org/ns/adres#huisnummer", "house number", numberObj));
            
            RDFObject cityObj = new RDFObject();
            cityObj.isLiteral = true;
            cityObj.value = "Olen";
            cityObj.label = "Olen";
            properties.add(new RDFPredicate("http://www.w3.org/ns/locn#geographicName", "city", cityObj));
            
            RDFObject postalCodeObj = new RDFObject();
            postalCodeObj.isLiteral = true;
            postalCodeObj.value = "2250";
            postalCodeObj.label = "2250";
            properties.add(new RDFPredicate("http://www.w3.org/ns/locn#postCode", "postal code", postalCodeObj));
    ...
    public void testRecursiveBlankNodeExpansion() {
        // Test case: Registration blank node (like in the example)
        ...
        assertNotNull(registrationObj.getNestedProperties(), "Registration blank node should have nested properties");
        assertFalse(registrationObj.getNestedProperties().isEmpty(), "Registration blank node should not have empty properties");
        assertEquals(2, registrationObj.getNestedProperties().size(), "Registration blank node should have 2 properties");
        ...
        for (RDFPredicate predicate : registrationObj.getNestedProperties()) {
            if (predicate.getLabel().equals("notation")) {
                ...
                assertEquals("2105029959", predicate.getObjects().get(0).getValue(), "Notation should have correct value");
            } else if (predicate.getLabel().equals("type")) {
                hasType = true;
                assertFalse(predicate.getObjects().get(0).isLiteral(), "Type should not be a literal");
                assertEquals("http://www.w3.org/ns/adms#Identifier", predicate.getObjects().get(0).getValue(), "Type should have correct URI");
            }
        }
        ...
        assertNotNull(addressObj.getNestedProperties(), "Address blank node should have nested properties");
        assertFalse(addressObj.getNestedProperties().isEmpty(), "Address blank node should not have empty properties");
        assertEquals(4, addressObj.getNestedProperties().size(), "Address blank node should have 4 properties");
        ...
        for (RDFPredicate predicate : addressObj.getNestedProperties()) {
            if (predicate.getLabel().equals("street name")) {
                ...
                assertEquals("Watertorenstraat", predicate.getObjects().get(0).getValue(), "Street should have correct value");
                ...
                assertEquals("33", predicate.getObjects().get(0).getValue(), "House number should have correct value");
            } else if (predicate.getLabel().equals("city")) {
                hasCity = true;
                assertTrue(predicate.getObjects().get(0).isLiteral(), "City should be a literal");
                assertEquals("Olen", predicate.getObjects().get(0).getValue(), "City should have correct value");
                ...
                assertEquals("2250", predicate.getObjects().get(0).getValue(), "Postal code should have correct value");
            }
        }
